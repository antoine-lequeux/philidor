...
inline bool is_in_check(const Board& board, Color color)
{
    Square ksq = board.kings[static_cast<usize>(color)];
    if (color == Color::WHITE)
        return is_attacked_by<Color::BLACK>(board, ksq, board.occupancy);
    else
        return is_attacked_by<Color::WHITE>(board, ksq, board.occupancy);
}
...
inline void generate_moves_impl(const Board& board, MoveList& ml)
{
    ...
    for (Square to : Bitloop(king_attacks(ksq) & ~our & filter))
    ...
    if constexpr (type == GenType::ALL || type == GenType::QUIETS)
    {
        for (Square to : Bitloop(single & ~PROMO_RANK & target_mask))
    ...
    for (Square from : Bitloop(pinned_pawns))
    {
        ...
        if constexpr (type == GenType::ALL || type == GenType::QUIETS || type == GenType::CAPTURES)
        {
            ...
            if ((empty & (1ULL << one)) && (pin_ray & (1ULL << one)) && (target_mask & (1ULL << one)))
            {
                ...
                else if constexpr (type == GenType::ALL || type == GenType::QUIETS)
                {
    ...
    for (Square from : Bitloop(board.piece_bb[bb_index(Type::KNIGHT, us_idx)] & not_pinned))
        add_moves(knight_attacks(from) & ~our & target_mask & filter, from, ml);

    const u64 bishops = board.piece_bb[bb_index(Type::BISHOP, us_idx)];
    for (Square from : Bitloop(bishops & not_pinned))
        add_moves(bishop_attacks(from, occ) & ~our & target_mask & filter, from, ml);
    for (Square from : Bitloop(bishops & pinned))
        add_moves(bishop_attacks(from, occ) & ~our & target_mask & line_through(ksq, from) & filter, from, ml);

    const u64 rooks = board.piece_bb[bb_index(Type::ROOK, us_idx)];
    for (Square from : Bitloop(rooks & not_pinned))
        add_moves(rook_attacks(from, occ) & ~our & target_mask & filter, from, ml);
    for (Square from : Bitloop(rooks & pinned))
        add_moves(rook_attacks(from, occ) & ~our & target_mask & line_through(ksq, from) & filter, from, ml);

    const u64 queens = board.piece_bb[bb_index(Type::QUEEN, us_idx)];
    for (Square from : Bitloop(queens & not_pinned))
        add_moves(queen_attacks(from, occ) & ~our & target_mask & filter, from, ml);
    for (Square from : Bitloop(queens & pinned))
        add_moves(queen_attacks(from, occ) & ~our & target_mask & line_through(ksq, from) & filter, from, ml);
}
} // namespace detail

template <GenType type>
inline void generate_moves_interface(const Board& board, MoveList& ml)
{
    if (board.side_to_move == Color::WHITE)
        detail::generate_moves_impl<Color::WHITE, type>(board, ml);
    else
        detail::generate_moves_impl<Color::BLACK, type>(board, ml);
}
